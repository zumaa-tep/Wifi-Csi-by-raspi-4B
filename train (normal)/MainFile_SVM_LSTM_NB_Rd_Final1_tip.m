...
EMPTY = dir('Train/Data8_Train1/room/Red/EMPTY');  % Load Emptay files
...
i
filename = ['Train/Data8_Train1/room/Red/EMPTY/' filepath1{i}];  %% Define this path here and in SVM_test.m file as well
...
for v=1:Number_VD_EMPTY_Files
    ...
    v
filename = ['Train/Data8_Train1/room/Red/EMPTY/' vd_EMPTY{v}];
...
SIT = dir('Train/Data8_Train1/room/Red/SIT');  % org
...
filename = ['Train/Data8_Train1/room/Red/SIT/' filepath2{i}];
% calling the CSIreader function to extract the packets data from the file
[csi_buff,NFFT] = CSIReader(filename,numpackets);
% calling the plotcsi function to plot the denoised data
output = plotcsi(csi_buff, NFFT, false);
...
for v=1:Number_VD_SIT_Files
    v
    % fetching each file name
filename = ['Train/Data8_Train1/room/Red/SIT/' vd_SIT{v}];
% calling the CSIreader function to extract the packets data from the file
[csi_buff,NFFT] = CSIReader(filename,numpackets);
% calling the plotcsi function to plot the denoised data
output_vd = plotcsi(csi_buff, NFFT, false);
...
%% Stand Data
STAND = dir('Train/Data8_Train1/room/Red/STAND');  % org
...
for i=1:Number_Stand_Files
    i
filename = ['Train/Data8_Train1/room/Red/STAND/' filepath3{i}];
[csi_buff,NFFT] = CSIReader(filename,numpackets);
output = plotcsi(csi_buff, NFFT, false);
pcacomponents = PCAcomponents(output);
...
for v=1:Number_VD_STAND_Files
    v
    % fetching each file name
filename = ['Train/Data8_Train1/room/Red/STAND/' vd_STAND{v}];
% calling the CSIreader function to extract the packets data from the file
[csi_buff,NFFT] = CSIReader(filename,numpackets);
% calling the plotcsi function to plot the denoised data
output_vd = plotcsi(csi_buff, NFFT, false);
...
FALL = dir('Train/Data8_Train1/room/Red/FALL');  % org
...
train_idx = sort(randperm(nFile,round(nFile*PD))); % calculate index for training files
test_idx = setxor(train_idx,1:nFile); % calculate index for Test files
filepath5 = FALL(train_idx,:);     % training indices %% แก้ชื่อหน่อยเดี่ญวตัวแปรซ้ำ
filepath5_Test_FALL = FALL(test_idx,:) ; % test indices %% แก้ชื่อหน่อยเดี่ญวตัวแปรซ

Number_FALL_Files = length(filepath3); % number of Experiment Files for train
Data_FALL = []; % matrix for SVM
% Data_Stand_m = []; % matrix for SVM with moment upto 5th order
for i=1:Number_FALL_Files
    i
filename = ['Train/Data8_Train1/room/Red/FALL/' filepath5{i}];
[csi_buff,NFFT] = CSIReader(filename,numpackets);
output = plotcsi(csi_buff, NFFT, false);
pcacomponents = PCAcomponents(output);
...
for v=1:Number_VD_FALL_Files
    v
    % fetching each file name
filename = ['Train/Data8_Train1/room/Red/FALL/' vd_FALL{v}];
% calling the CSIreader function to extract the packets data from the file
[csi_buff,NFFT] = CSIReader(filename,numpackets);
% calling the plotcsi function to plot the denoised data
output_vd = plotcsi(csi_buff, NFFT, false);
Data_VD_LSTM{v+x,1} = output_vd'; % Data_Train LSTM Raw    
LabelsValidation{v+x} = 'FALL';
end
z=y+v;

%% Walk Data
WALK = dir('Train/Data8_Train1/room/Red/WALK');  % org
...
for i=1:Number_Walk_Files
    i
filename = ['Train/Data8_Train1/room/Red/WALK/' filepath4{i}];
[csi_buff,NFFT] = CSIReader(filename,numpackets);
output = plotcsi(csi_buff, NFFT, false);
pcacomponents = PCAcomponents(output);
...
Data_Train{i+n,1} = FeatureExtraction(pcacomponents);
% Data_Train_m{i+m,1} = FeatureExtraction2(pcacomponents);
Data_Train_LSTM{i+n,1} = output'; % Data_Train LSTM Raw
Labels_Train{i+n} = 'Walk';
end
...
for v=1:Number_VD_WALK_Files
    v
    % fetching each file name
filename = ['Train/Data8_Train1/room/Red/WALK/' vd_WALK{v}];
% calling the CSIreader function to extract the packets data from the file
[csi_buff,NFFT] = CSIReader(filename,numpackets);
% calling the plotcsi function to plot the denoised data
output_vd = plotcsi(csi_buff, NFFT, false);
Data_VD_LSTM{v+z,1} = output_vd'; % Data_Train LSTM Raw    
LabelsValidation{v+z} = 'Walk';
end
...
for i=1:Number_Empty_Files
    i
filename = ['Train/Data8_Train1/room/Red/EMPTY/' filepath1_Test_Empty{i}];
[csi_buff,NFFT] = CSIReader(filename,numpackets);
output = plotcsi(csi_buff, NFFT, false);
pcacomponents = PCAcomponents(output);
...
for i=1:Number_Sit_Files
    i
filename = ['Train/Data8_Train1/room/Red/SIT/' filepath2_Test_SIT{i}];
[csi_buff,NFFT] = CSIReader(filename,numpackets);
output = plotcsi(csi_buff, NFFT, false);
pcacomponents = PCAcomponents(output);
...
for i=1:Number_Stand_Files
    i
filename = ['Train/Data8_Train1/room/Red/STAND/' filepath3_Test_STAND{i}];
[csi_buff,NFFT] = CSIReader(filename,numpackets);
output = plotcsi(csi_buff, NFFT, false);
pcacomponents = PCAcomponents(output);
...
for i=1:Number_FALL_Files
    i
filename = ['Train/Data8_Train1/room/Red/FALL/' filepath5_Test_FALL{i}];
[csi_buff,NFFT] = CSIReader(filename,numpackets);
output = plotcsi(csi_buff, NFFT, false);
pcacomponents = PCAcomponents(output);
Data_Test{i+l,1} = FeatureExtraction(pcacomponents);
...
for i=1:Number_Walk_Files
    i
filename = ['Train/Data8_Train1/room/Red/WALK/' filepath4_Test_WALK{i}];
[csi_buff,NFFT] = CSIReader(filename,numpackets);
output = plotcsi(csi_buff, NFFT, false);
pcacomponents = PCAcomponents(output);
...
[Data_Test_SVM,Labels_Test_SVM ] = SVM_Data(Data_Train_SVM, Labels_Train_SVM, filepath1_Test_Empty, numpackets, ...
    filepath2_Test_SIT, filepath3_Test_STAND, filepath4_Test_WALK , filepath5_Test_FALL ); %%% 
...
[trainedClassifierSVM, ~] = SVM_trainClassifier_Quard_Opt(Data_Train_SVM, Labels_Train);
...
figure;
confusion_matrix_NB = coxnfusionmat(Labels_Test_SVM,NBfit);
% Visualizing the confusion matrix
